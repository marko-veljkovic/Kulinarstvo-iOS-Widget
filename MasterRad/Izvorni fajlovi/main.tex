...
\kljucnereci{виџет,  \textit{Swift}, \textit{iOS}, \textit{Apple}, програмски језик, програмирање}
...
\begin{itemize}
   \item Једини тернарни оператор који постоји у језику \textit{Swift} је оператор '?:'. Код овог оператора израз са крајње леве стране мора бити типа \ref{subsec:Контрола тока} - \nameref{subsec:Контрола тока}) који једна линија са тернарним оператором може заменити приказани су у примеру \ref{lst:Тернарни оператор} - \nameref{lst:Тернарни оператор}.
...
\indent \textit{Xcode} се састоји од неколико алата који помажу програмеру приликом развоја апликација за \textit{Apple} платформе, од креирања апликације, преко тестирања и оптимизације, до прослеђивања на \textit{App Store}-у. Најзначајнији алати који су део \textit{Xcode}-а су симулатор и инструменти.
...
\\
\indent Као што је већ истакнуто, симулатор је део \textit{Xcode}-а; инсталира се уз њега, а покреће се и понаша као апликација оперативног система \textit{macOS} и омогућава симулацију свих уређаја са \textit{Apple} платформе (\textit{iPhone}, \textit{iPad}, \textit{Apple Watch}, \textit{Apple TV}). Приликом тестирања могуће је и покретање више симулатора за различите платформе да би се тестирала њихова компатибилност, као на пример сарадња апликације на \textit{iPhone}-у и \textit{Apple Watch}-у. 
...
\indent Виџет, као део мобилне апликациjе, се налази на почетном екрану уређаjа (телефона или таблета) и кориснику приказуjе одабране важне информациjе из те апликациjе. За разлику од виџета у оперативном систему \textit{Android}, коjи су присутни више од десет година, виџети на Apple платформама су уведени 2020. године, тако да jе и сама технологиjа коjа подржава њихово креирање и даље у активном развоjу.

\section{Основно}

\indent Виџет на уређајима са \textit{Apple} платформом узима један од кључних делова апликације за коју је развијен и приказе га крајњим корисницима тамо где ће га најлакше уочити, на \textit{iPhone}-у и \textit{iPad}-у се може налазити на почетном екрану или у делу \textit{Today View}-а, док се на \textit{Mac} уређајима налази у центру за нотификације. Величина виџета није флексибилна као на \textit{Android} уређајима, па тако постоји могућност креирања малих (величина 2x2 места на почетном екрану \textit{iPhone}-а), средњих (2x4) и великих (4x4), а од верзије оперативног система \textit{iPadOS15} екстра великих (4x8) (само за \textit{iPad} уређаје) виџета. 
\\
\indent Скуп свих тренутно доступних виџета на уређају налази се у галерији виџета (енг. \textit{widget gallery}), која помаже корисницима приликом одабира конкретне величине и типа виџета (једна апликација може испоручити више типова виџета исте величине). Унутар галерије такође постоји опција за измену виџета у којој корисници могу да контролишу и мењају своје виџете и тиме их прилагоде себи, али само уколико је у току конструисања виџета од стране програмера то омогућено. Више речи о овоме биће у делу \ref{sec:Развој виџетa} - \nameref{sec:Развој виџетa}.
\\
...
\indent Као што је већ напоменуто, шаблон виџет додатка пружа иницијалну имплементацију виџета која имплементира \textit{Widget} протокол. Два могућа начина конфигурације виџета су статичка (енг. \textit{StaticConfiguration}) и конфигурација са сврхом (енг. \textit{IntentConfiguration}). \\
\indent Статичка конфигурација се користи за виџете који немају параметре који могу бити конфигурисани од стране корисника (на пример, системска апликација \textit{Screen time} која води статистику о времену проведеном на одговарајућем уређају). Конфигурација са сврхом се користи за виџете чији одређени параметри могу бити конфигурисани од стране корисника (на пример, системска апликација за временску прогнозу где корисник може наместити одређени град за који жели да добија податке). Ова конфигурација ће бити укључена и конфигурациони фајл ће бити додат уколико програмер приликом додавања виџет додатка штиклира поље \textit{Include Configuration Intent}.
\\
...
\begin{itemize}
    ...
    \item Снабдевач (енг. \textit{Provider}), објекат класе која имплементира протокол \textit{TimelineProvider} и кроз временску линију коју производи одређује у ком тренутку ће виџет бити поново изрендерован и нови подаци бити приказани. Више о овом протоколу и свеукупној причи о временој линији у делу \ref{subsec:Временска линија} - \nameref{subsec:Временска линија}
    ...
    \item Прилагођена сврха (енг. \textit{Custom Intent}), фајл који дефинише параметре које корисник може мењати и прилагођавати себи, више о овоме у делу: \ref{subsec:Intent} - \nameref{subsec:Intent}
\end{itemize}
...
\\
\indent Да би виџет био приказан у \textit{widget} галерији, \textit{WidgetKit} захтева од снабдевача преглед снимка (енг. \textit{Preview snapshot}). Дохватање прегледа снимка се разрешава коришћењем променљиве '\textit{isPreview}' којом се проверава да ли снабдевач прегледа снимка шаље тренутни снимак за приказ у галерији или за приказ виџета на почетном екрану. Када је параметар '\textit{isPreview}' тачан, виџет се приказује у галерији. Уколико за приказ виџета треба да буду приказани и одређени подаци, а подаци нису пристигли са серверске стране, постоје два решења. Могу се приказати подразумевани, унапред одређени подаци, или се могу користити подаци који чувају место правим подацима (енг. \textit{placeholder}). У примеру \ref{lst:виџет - placeholder} - \nameref{lst:виџет - placeholder} може се видети креирање погледа чувара места (енг. \textit{placeholder view}) коришћењем статичких података који су увек доступни, и конкретан приказ тог чувара места у прегледу уживо са сакривеним подацима (енг. \textit{redacted data}) - приказ како ће корисник видети виџет док не пристигну конкретни подаци у неким ситуацијама.
...
\label{subsec:Intent}
\indent Виџети представљају погледе који не интерагују са корисницима, односно не подржавају интерактивне елементе, као што су поглед \textit{Scroll} и дугме \textit{Switch}. Једна врста интеракције корисника са виџетом постиже се омогућавањем конфигурације виџета од стране корисника коришћењем конфигурације \textit{Intent}, у којој се наводе сви параметри које корисник може да промени (и дозвољене вредности за те параметре). 
\\
...
\indent Једини начин директне комуникације између корисника и виџета остварена је везама (енг. \textit{links}) унутар виџета. Када корисник кликне на виџет отвара се апликација којој тај виџет припада, и може се конфигурисати који део апликације ће бити приказан кориснику у зависности од елемента унутар виџета на који је кликнуо. Свим величинама виџета може бити додат модификатор \textit{widgetURL(\_:)}, којим се одређује у који део апликације ће корисник бити одведен када кликне на виџет.
\\
\indent За све величине виџета, осим малих, може се користити и веза (енг. \textit{Link}) која се додаје једном погледу унутар виџета, којом је одређено место у апликацији које ће бити отворено (на пример, један виџет средње величине који садржи листу са 3 рецепата, сваки елемент листе има везу која води ка детаљној страни о рецепту који тај елемент представља). Иако виџет користи везе унутар својих елемената, може користити и модификатор \textit{widgetURL(\_:)}. Овај модификатор ће бити активиран уколико корисник кликне на поглед унутар виџета који нема дефинисану везу и биће отворена апликација без додатне навигације од стране виџета. У примеру кода \ref{lst:Везе унутар виџета} - \nameref{lst:Везе унутар виџета} приказана је употреба модификатора \textit{widgetURL(\_:)} за мале виџете, као и употреба веза за средње и велике виџете.
...
\subsection{Више виџета у једном проширењу}
\indent Уколико постоји потреба за коришћењем више различитих типова виџета у једном проширењу, то се може лако постићи уз само пар измена главног дела проширења, означеног анотацијом \textit{@main}. Уместо протокола \textit{Widget} главна структура мора имплементирати протокол \textit{WidgetBundle}. Тело структуре сада имплементира протокол \textit{Widget} и додата је анотација \textit{@WidgetBundleBuilder}. Приказ употребе више типова виџета у једном проширењу може се видети у примеру \ref{lst:Више виџета у једном проширењу} - \nameref{lst:Више виџета у једном проширењу}.

\begin{lstlisting}[caption=\textit{{Више виџета у једном проширењу}}, label={lst:Више виџета у једном проширењу}, language=Swift, frame=single]
    @main
...
\indent Да би виџети могли да пружају корисне и прецизне информације морају бити ажурирани. Виџети не подржавају ажурирање у реалном времену, а и сам систем може ограничити ажурирање виџета у завиности од корисничког понашања и интеракције са њим, па се мора пронаћи начин на који ће подаци у виџету увек бити релевантни. 
\\
...
\indent У већини случајева виџет треба да омогући кориснику конфигурабилност како би могао да пружи релевантне информације (на пример, књига коју корисник тренутно чита и његов прогрес у апликацији \textit{Apple Books}), док поједини виџети могу то изоставити (на пример, најновије вести). Уколико је виџет конфигурабилан, потребно је да подешавања буду једноставна и да се не захтева превише информација од корисника. Кориснички интерфејс за измену виџета је унапред одређен и исти за све виџете, као што је показано у делу \ref{subsec:Intent} - \nameref{subsec:Intent}.

\subsection{Дизајн прилагођен свима}
\indent Виџети треба да буду јарких боја како би се истицали на екрану, али истовремено и јасно видљив текст како би корисник могао да види све потребне информације након откључавања или непосредно пре закључавања уређаја. Виџет треба прилагодити апликацији коју представља (боје, фонт текста, јединствени елементи...), док истовремено не треба истицати превише елемената који ће указивати на апликацију (лого, име) јер се тиме само непотребно заузима простор унутар виџета који се може боље искористити.  
\\
\indent Количина информација која ће бити приказана у виџету мора бити оптимална. Уколико се прикаже премало информација виџет неће имати превелики значај за кориснике, док превише информација на мало простора отежава читање и разумевање података.
\\
\indent Једна од ставки која се не сме заборавити у данашње време је израда дизајна виџета за обе врсте боја системске позадине (светле и тамне). Дизајн виџета се не сме разликовати од системске боје позадине јер ниједан корисник не жели видети таман текст на светлој боји позадине уколико је изабрао тамну системску боју позадине. Приликом израде обе врсте дизајна може помоћи \textit{Xcode preview} који омогућава истовремено сагледавање оба дизајна, упоређивање и исправљање евентуалних недостатака.
\\
\indent \textit{Apple} саветује да се никад не користи фонт текста мањи од 11 поена\footnote{\textit{Apple}-ов израз за "број који треба уписати у поље", универзална мера у дизајну на \textit{Apple} платформама}. Коришћење мањег фонта би корисницима знатно отежало употребу виџета. Увек треба користити званичне елементе за приказ текста, како би се омогућила скалабилност као и системско читање текста. 
\\
...
\\
\indent Рад апликације ће бити приказан упоредо на два симулатора, \textit{iPhone 13 Pro Max} са тамном бојом позадине (у наставку означен бројем 1) и \textit{iPhone SE (2nd generation)} са светлом бојом позадине (у наставку означен бројем 2). Оба симулатора покрећу оперативни систем \textit{iOS 15.0}.
...
\indent Рецепти су приказани унутар табеле, сваки рецепт је презентован именом, временом припреме у минутима и сликом. Приказ почетног екрана може се видети на сликама \ref{slika:почетни_екран_1} и \ref{slika:почетни_екран_2}.

\begin{figure} %[H] da li?
    \centering
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - overview.png} 
        \caption{\textit{Почетни екран - 1}}
        \label{slika:почетни_екран_1}
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - overview.png} 
        \caption{\textit{Почетни екран - 2}}
        \label{slika:почетни_екран_2}
    \end{minipage}
\end{figure}

\indent Корисник може изабрати одређену категорију из које ће му бити приказани рецепти. Међу понуђеним категоријама се налазе "Хладно предјело", "Топло предјело", "Главно јело", "Ужина", "Пиће", "Супе и чорбе", "Дезерт", "Салата" и "Хлеба". Приказ избора категорије налази се на сликама \ref{slika:категорија_1} и \ref{slika:категорија_2}, док је на сликама \ref{slika:изабрана_категорија_1} и \ref{slika:изабрана_категорија_2} приказан изглед екрана када је једна категорија изабрана (конкретно категорија "Главно јело").

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - category.png} 
        \caption{\textit{Категорија - 1}}
        \label{slika:категорија_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - category.png} 
        \caption{\textit{Категорија - 2}}
        \label{slika:категорија_2}
    \end{minipage}
\end{figure}
...
\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - choosen category.png} 
        \caption{\textit{Изабрана категорија - 1}}
        \label{slika:изабрана_категорија_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - choosen category.png} 
        \caption{\textit{Изабрана категорија - 2}}
        \label{slika:изабрана_категорија_2}
    \end{minipage}
\end{figure}

\indent Поред избора категорије, корисник може филтрирати приказане рецепате претрагом по имену и у том случају ће му бити приказани сви рецепти чије име садржи текст који је корисник унео у поље претраге. Почетак претраге приказан је на сликама \ref{slika:претрага_1} и \ref{slika:претрага_2}. 
\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - search.png} 
        \caption{\textit{Претрага - 1}}
        \label{slika:претрага_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - search.png} 
        \caption{\textit{Претрага - 2}}
        \label{slika:претрага_2}
    \end{minipage}
\end{figure}

\indent Рецепте на почетној страни корисник може и сортирати, опадајуће или растуће по укупном времену потребном за њихово спремање (припрема и кување). Сортирање се може и комбиновати са избором категорије или претрагом по називу, па тако корисник лако може наћи рецепт из одређене категорије (на пример, из категорије дезерта) који се спрема најбрже од понућених. Сортирани рецепти у растућем редоследу су приказани на сликама \ref{slika:сортирање_растуће_1} и \ref{slika:сортирање_растуће_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - sort asc.png} 
        \caption{\textit{Рецепти сортирани растуће - 1}}
        \label{slika:сортирање_растуће_1}
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - sort asc.png} 
        \caption{\textit{Рецепти сортирани растуће - 2}}
        \label{slika:сортирање_растуће_2}
    \end{minipage}
\end{figure}
...
%    \begin{minipage}{0.5\textwidth}
%        \centering
%        \includegraphics[width=0.95\textwidth]{images/simulators/dar%k - sort desc.png} 
%        \caption{\textit{Рецепти сортирани опадајуће - 1}}
%        \label{slika:сортирање_опадајуће_1}
%    \end{minipage}\hfill
%    \begin{minipage}{0.5\textwidth}
%        \centering
%        \includegraphics[width=0.95\textwidth]{images/simulators/lig%ht - sort desc.png} 
%        \caption{\textit{Рецепти сортирани опадајуће - 2}}
%        \label{slika:сортирање_опадајуће_2}
%    \end{minipage}
%\end{figure}
...
\indent "Омиљени рецепти" је део апликације који садржи листу свих рецепата које је корисник означио као омиљене и тиме их издвојио од осталих. Слике \ref{slika:омиљени_1} и \ref{slika:омиљени_2} представљају изглед дела "Омиљени рецепти".

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - favorites.png} 
        \caption{\textit{Омиљени рецепти - 1}}
        \label{slika:омиљени_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - favorites.png} 
        \caption{\textit{Омиљени рецепти - 2}}
        \label{slika:омиљени_2}
    \end{minipage}
\end{figure}

\indent Када корисник дода свој рецепт, он ће бити приказан у делу "Моји рецепти" у којем ће корисник моћи да види све своје рецепте са којима може и да манипулише, више о овоме биће објашњено у делу \ref{subsec:Измена и брисање постојећег рецепта} - \nameref{subsec:Измена и брисање постојећег рецепта}. Још једна могућност која се пружа кориснику на овој страни је додавање новог рецета, што ће детаљно бити објашњено у делу \ref{subsec:Креирање новог рецепта} - \nameref{subsec:Креирање новог рецепта}. Сликама \ref{slika:моји_рецепти_1} и \ref{slika:моји_рецепти_2} приказан је изглед странице "Моји рецепти".

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - my recipes.png} 
        \caption{\textit{Моји рецепти - 1}}
        \label{slika:моји_рецепти_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - my recipes.png} 
        \caption{\textit{Моји рецепти - 2}}
        \label{slika:моји_рецепти_2}
    \end{minipage}
\end{figure}
...
\indent У детаљном приказу рецепта, кориснику је презентован опис рецепта који се састоји од: назива рецепта, категорије којој рецепт припада, слике рецепта, времена припреме и спремања, могућност повећавања и смањивања броја особа за које је рецепт предвиђен (истовремено ће бити промењена количина састојака као и време припреме), списак састојака и корака припреме, могућност додавања и брисања рецепта из листе омиљених, и уколико је приказани рецепт креиран од стране тренутног корисника, може га изменити или обрисати. Детаљан приказ рецепта за цезар салату приказан је на сликама \ref{slika:детаљан_рецепт_1} и \ref{slika:детаљан_рецепт_2} за четири особе односно на сликама \ref{slika:детаљан_рецепт_2_1} и \ref{slika:детаљан_рецепт_2_2} за шест особа.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - detail4.png} 
        \caption{\textit{Детаљан приказ рецепта - 1}}
        \label{slika:детаљан_рецепт_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - detail4.png} 
        \caption{\textit{Детаљан приказ рецепта - 2}}
        \label{slika:детаљан_рецепт_2}
    \end{minipage}
\end{figure}
...
\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - detail6.png} 
        \caption{\textit{Детаљан приказ рецепта за 6 особа - 1}}
    ...
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - detail4.png} 
        \caption{\textit{Детаљан приказ рецепта за 6 особа - 2}}
        \label{slika:детаљан_рецепт_2_2}
    \end{minipage}
\end{figure}
...
\indent Приликом креирања новог рецепта од корисника се тражи да унесе све потребне информације које су наведене у претходном поглављу. Изглед погледа додавања новог рецепта може се видети на сликама \ref{slika:нов_рецепт_1} и \ref{slika:нов_рецепт_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - new.png} 
        \caption{\textit{Нов рецепт - 1}}
        \label{slika:нов_рецепт_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - new.png} 
        \caption{\textit{Нов рецепт - 2}}
        \label{slika:нов_рецепт_2}
    \end{minipage}
\end{figure}
...
\indent Корисник има могућност промене свих рецепата које је додао. Приказ изгледа погледа измене рецепта налази се на сликама \ref{slika:измена_рецепта_1} и \ref{slika:измена_рецепта_2}. Постоји могућност измене и брисања састојака \ref{slika:измена_састојака_1} и \ref{slika:измена_састојака_2}, додавања нових састојака \ref{slika:измена_састојака_2_1} и \ref{slika:измена_састојака_2_2}, измене и брисања корака припреме \ref{slika:измена_корака_1} и \ref{slika:измена_корака_2} и додавања нових корака припреме \ref{slika:измена_корака_2_1} и \ref{slika:измена_корака_2_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - change.png} 
        \caption{\textit{Измена рецепта - 1}}
        \label{slika:измена_рецепта_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - change.png} 
        \caption{\textit{Измена рецепта - 2}}
        \label{slika:измена_рецепта_2}
    \end{minipage}
\end{figure}
...
\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - ingredients1.png} 
        \caption{\textit{Измена састојака - 1}}
        \label{slika:измена_састојака_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - ingredients1.png} 
        \caption{\textit{Измена састојака - 2}}
        \label{slika:измена_састојака_2}
    \end{minipage}
\end{figure}
...
\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - ingredients2.png} 
        \caption{\textit{Измена састојака са новим састојком - 1}}
        \label{slika:измена_састојака_2_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - ingredients2.png} 
        \caption{\textit{Измена састојака са новим састојком - 2}}
        \label{slika:измена_састојака_2_2}
    \end{minipage}
\end{figure}
...
\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - steps1.png} 
        \caption{\textit{Измена корака - 1}}
        \label{slika:измена_корака_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - steps1.png} 
        \caption{\textit{Измена корака - 2}}
        \label{slika:измена_корака_2}
    \end{minipage}
\end{figure}
...
\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - steps2.png} 
        \caption{\textit{Измена корака са новим кораком - 1}}
        \label{slika:измена_корака_2_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - steps2.png} 
        \caption{\textit{Измена корака са новим кораком - 2}}
        \label{slika:измена_корака_2_2}
    \end{minipage}
\end{figure}

\indent Уколико корисник жели може и обрисати рецепте које је додао, кликом на дугме "Обриши рецепт" унутар детаљног приказа рецепта, након чега ће му бити приказано упозорење у којем ће моћи да потврди брисање рецепта или да од њега одустане. Приказ упозорења приликом брисања рецепта налази се на сликама \ref{slika:брисање_рецепта_1} и \ref{slika:брисање_рецепта_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - delete.png} 
        \caption{\textit{Брисање рецепта - 1}}
        \label{slika:брисање_рецепта_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - delete.png} 
        \caption{\textit{Брисање рецепта - 2}}
        \label{slika:брисање_рецепта_2}
    \end{minipage}
\end{figure}

\subsection{Приказ виџета}

\indent Корисник може додати виџет на почетни екран из виџет галерије, где се налазе оба типа виџета ове апликације (први тип у све три величине, док је други представљен само у великој величини) који ће детаљније бити објашњени у наставку. Приказ додавања виџета из галерије представљен је на сликама \ref{slika:додавање_виџета_1} и \ref{slika:додавање_виџета_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - add widget.png} 
        \caption{\textit{Додавање виџета - 1}}
        \label{slika:додавање_виџета_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - add widget.png} 
        \caption{\textit{Додавање виџета - 2}}
        \label{slika:додавање_виџета_2}
    \end{minipage}
\end{figure}

\indent Први тип виџета (назван "Рецепт на клик") доступан је корисницима у све три величине (мала, средња и велика). Мали виџет приказује слику рецепта уз његов назив и може послужити као подсетник кориснику шта је испланирао да спрема или као пречица ка детаљном опису тог рецепта. Средњи виџет је проширење малог виџета који додатно приказује састоје или кораке припреме рецепта, параметар који је конфигурабилан од стране корисника. Приказ малог и средњег виџета може се видети на сликама \ref{slika:приказ_виџета_1_1} и \ref{slika:приказ_виџета_1_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - widgets 1-1.png} 
        \caption{\textit{Приказ виџета мале и средње величине - 1}}
        \label{slika:приказ_виџета_1_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - widgets 1-1.png} 
        \caption{\textit{Приказ виџета мале и средње величине - 2}}
        \label{slika:приказ_виџета_1_2}
    \end{minipage}
\end{figure}

\indent Први тип виџета у великој величини приказује рецепт са листом састојака и корака припреме, који су као код средњег виџета конфигурабилни и корисник може изабрати који ће од параметара бити примаран (приказан у дужој листи), уколико тај параметар испуњава услов (дужина листе мора бити већа од шест елемената). Велики виџет првог типа приказан је на сликама \ref{slika:приказ_виџета_2_1} и \ref{slika:приказ_виџета_2_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - widgets 1-2.png} 
        \caption{\textit{Приказ првог виџета велике величине - 1}}
        \label{slika:приказ_виџета_2_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - widgets 1-2.png} 
        \caption{\textit{Приказ првог виџета велике величине - 2}}
        \label{slika:приказ_виџета_2_2}
    \end{minipage}
\end{figure}

\indent Први тип виџета је конфигурабилан, односно корисник може мењати одређене параметре. Код малог виџета може променити рецепт који ће му бити приказан и изабрати неки од рецепата из своје листе омиљених рецепата. Средњи и велики виџет поред избора рецепта омогућавају и одабир параметра који одређује која ће листа бити примарна (листа састојака или листа корака припреме). Изглед екрана приликом конфигурације средњег виџета приказан је на сликама  \ref{slika:измена_виџета_1} и \ref{slika:измена_виџета_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - change widget.png} 
        \caption{\textit{Измена виџета - 1}}
        \label{slika:измена_виџета_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - change widget.png} 
        \caption{\textit{Измена виџета - 2}}
        \label{slika:измена_виџета_2}
    \end{minipage}
\end{figure}

\indent Други тип виџета се разликује од првог по неколико карактеристика. Други тип је доступан само у великој величини, није га могуће конфигурисати и приказује четири рецепта. Рецепти који су приказани у овом типу виџета представљени су као скуп четири мала виџета првог типа, који су статички конфигурисани и поновно се учитавају свака 24 сата и тако кориснику предлажу шта би могао да спрема тог дана. Приказани рецепти се налазе у листи омиљених рецепата корисника, тако да је задовољство корисника изабраним рецептима загарантовано. Приказ другог типа виџета може се видети на сликама \ref{slika:приказ_виџета_3_1} и \ref{slika:приказ_виџета_3_2}.

\begin{figure}
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=0.95\textwidth]{images/simulators/dark - widgets 2-1.png} 
        \caption{\textit{Приказ другог виџета велике величине - 1}}
        \label{slika:приказ_виџета_3_1}
        \centering
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{images/simulators/light - widgets 2-1.png}
        \caption{\textit{Приказ другог виџета велике величине - 2}}
        \label{slika:приказ_виџета_3_2}
    \end{minipage}
\end{figure}

%\subsection{Даљи планови и могућа унапређења}
